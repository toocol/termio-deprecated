...
class HistoryFile {
 ...
 private:
  ...
  // when readWriteBalance goes below this threshold, the file will be mmap'ed
  // automatically
...
class CharacterFormat {
 public:
  bool equalsFormat(const CharacterFormat& other) const {
    return other.rendition == rendition && other.fgColor == fgColor &&
...
class CompactHistoryBlock {
 public:
  CompactHistoryBlock() {
    ...
    head = (quint8*)mmap(nullptr, blockLength, PROT_READ | PROT_WRITE,
