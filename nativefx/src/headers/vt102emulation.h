...
#define MODE_Mouse1006 \
  (MODES_SCREEN + 8)  // 2nd Xterm-style extended coordinates
#define MODE_Mouse1015 (MODES_SCREEN + 9)  // Urxvt-style extended coordinates
#define MODE_Ansi \
  (MODES_SCREEN + 10)  // Use US Ascii for character sets G0-G3 (DECANM)
#define MODE_132Columns \
  (MODES_SCREEN + 11)  // 80 <-> 132 column mode switch (DECCOLM)
#define MODE_Allow132Columns (MODES_SCREEN + 12)  // Allow DECCOLM mode
#define MODE_BracketedPaste \
  (MODES_SCREEN + 13)  // Xterm-style bracketed paste mode
 ...
 * http://rtfm.etla.org/xterm/ctlseq.html for a description of xterm's escape
 * sequences.
 *
 */
