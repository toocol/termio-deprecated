...
class ColorEntry {
 public:
   ...
   * @param tr Specifies that the color should be transparent when used as a
   * background color.
   * @param weight Specifies the font weight to use when drawing text with this
   * color.
   */
   ...
   * This is not applicable when the color is used to draw a character's
   * background.
   */
...
class CharacterColor {
 ...
 public:
  /** Constructs a new CharacterColor whoose color and color space are
   * undefined. */
   ...
   *
   * The @p palette is only used if this color is one of the 16 system colors,
   * otherwise it is ignored.
   */
  QColor color(const ColorEntry* palette) const;

  /**
   * Compares two colors and returns true if they represent the same color value
   * and use the same color space.
   */
  friend bool operator==(const CharacterColor& a, const CharacterColor& b);
  /**
   * Compares two colors and returns true if they represent different color
   * values or use different color spaces.
   */
  friend bool operator!=(const CharacterColor& a, const CharacterColor& b);
...
inline bool operator==(const CharacterColor& a, const CharacterColor& b) {
  return a._colorSpace == b._colorSpace && a._u == b._u && a._v == b._v &&
         a._w == b._w;
}
inline bool operator!=(const CharacterColor& a, const CharacterColor& b) {
  return !operator==(a, b);
