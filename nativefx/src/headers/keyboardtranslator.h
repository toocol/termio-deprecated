...
class KeyboardTranslator {
 public:
   ...
   *
   * This enum describes the states which may be associated with with a
   * particular entry in the keyboard translation entry.
   */
  enum State {
  ...
  class Entry {
   public:
     ...
     *
     * TODO: The numbers used to replace '*' characters are taken from the
     * Konsole/KDE 3 code. Document them.
     *
     ...
     */
    QByteArray escapedText(
        bool expandWildCards = false,
        Qt::KeyboardModifiers modifiers = Qt::NoModifier) const;

    /** Returns the character code ( from the Qt::Key enum ) associated with
     * this entry */
    int keyCode() const;
     ...
     * modifiers(), this means that the entry only matches when that modifier is
     * NOT pressed.
     *
     ...
     * terminal is in that state or not.
     */
    States state() const;
    ...
    /**
     * Returns this entry's conditions ( ie. its key code, modifier and state
     ...
     */
    bool matches(int keyCode, Qt::KeyboardModifiers modifiers,
                 States flags) const;
   ...
   */
  Entry findEntry(int keyCode, Qt::KeyboardModifiers modifiers,
...
class KeyboardTranslatorReader {
 ...
 private:
  ...
  void readNext();
  bool decodeSequence(const QString&, int& keyCode,
...
class KONSOLEPRIVATE_EXPORT KeyboardTranslatorManager {
 public:
  ...
  KeyboardTranslatorManager& operator=(const KeyboardTranslatorManager&) =
      delete;
  ...
  /**
   * Deletes a translator.  Returns true on successful deletion or false
   * otherwise.
   *
   * TODO: More documentation
   */
 ...
 private:
  ...
  KeyboardTranslator* loadTranslator(
      const QString& name);  // loads the translator
                             // with the given name
  ...
  QHash<QString, KeyboardTranslator*>
      _translators;  // maps translator-name -> KeyboardTranslator
                     // instance
  bool _haveLoadedAll;
};
...
inline void KeyboardTranslator::Entry::setModifiers(
...
inline void KeyboardTranslator::Entry::setModifierMask(
...
inline int oneOrZero(int value) { return value ? 1 : 0; }
inline QByteArray KeyboardTranslator::Entry::text(
  ...
  if (expandWildCards) {
    ...
    for (int i = 0; i < _text.length(); i++) {
      if (expandedText[i] == '*') expandedText[i] = '0' + modifierValue;
    }
  }
...
Q_DECLARE_METATYPE(TConsole::KeyboardTranslator::Entry)
Q_DECLARE_METATYPE(const TConsole::KeyboardTranslator*)
