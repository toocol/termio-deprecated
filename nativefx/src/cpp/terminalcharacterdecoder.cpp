...
void PlainTextDecoder::begin(QTextStream* output) {
  _output = output;
  if (!_linePositions.isEmpty()) _linePositions.clear();
}
void PlainTextDecoder::end() { _output = nullptr; }

void PlainTextDecoder::setRecordLinePositions(bool record) {
  _recordLinePositions = record;
}
QList<int> PlainTextDecoder::linePositions() const { return _linePositions; }
void PlainTextDecoder::decodeLine(const Character* const characters, int count,
  ...
  for (int i = 0; i < outputCount;) {
    ...
    i += qMax(1, wcwidth(characters[i].character));
  }
...
void HTMLDecoder::decodeLine(const Character* const characters, int count,
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (characters[i].rendition != _lastRendition ||
      ...
      if (_innerSpanOpen) closeSpan(text);
      ...
      if (useBold) style.append(QLatin1String("font-weight:bold;"));
...
void HTMLDecoder::openSpan(std::wstring& text, const QString& style) {
  text.append(
      QString(QLatin1String("<span style=\"%1\">")).arg(style).toStdWString());
}

void HTMLDecoder::closeSpan(std::wstring& text) { text.append(L"</span>"); }
