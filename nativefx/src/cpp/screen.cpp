...
Screen::Screen(int l, int c)
  ...
  for (int i = 0; i < lines + 1; i++) lineProperties[i] = LINE_DEFAULT;
...
void Screen::deleteChars(int n) {
  ...
  if (cuX >= screenLines[cuY].count()) return;

  if (cuX + n > screenLines[cuY].count()) n = screenLines[cuY].count() - cuX;
...
void Screen::insertChars(int n) {
  ...
  if (screenLines[cuY].size() < cuX) screenLines[cuY].resize(cuX);

  screenLines[cuY].insert(cuX, n, ' ');

  if (screenLines[cuY].count() > columns) screenLines[cuY].resize(columns);
}
...
void Screen::updateEffectiveRendition() {
  ...
  if (currentRendition & RE_BOLD) effectiveForeground.setIntensive();
}
...
void Screen::backspace() {
  ...
  if (screenLines[cuY].size() < cuX + 1) screenLines[cuY].resize(cuX + 1);
...
void Screen::scrollUp(int from, int n) {
  if (n <= 0) return;
  if (from > _bottomMargin) return;
  if (from + n > _bottomMargin) n = _bottomMargin + 1 - from;
...
void Screen::scrollDown(int from, int n) {
  ...
  if (n <= 0) return;
  if (from > _bottomMargin) return;
  if (from + n > _bottomMargin) n = _bottomMargin - from;
...
void Screen::clearImage(int loca, int loce, char c) {
  ...
  for (int y = topLine; y <= bottomLine; y++) {
    ...
    if (isDefaultCh && endCol == columns - 1) {
      ...
      if (line.size() < endCol + 1) line.resize(endCol + 1);

      Character* data = line.data();
      for (int i = startCol; i <= endCol; i++) data[i] = clearCh;
    }
  }
}
...
void Screen::moveImage(int dest, int sourceBegin, int sourceEnd) {
  ...
  if (lastPos != -1) {
    ...
    lastPos += diff;
    if ((lastPos < 0) || (lastPos >= (lines * columns))) lastPos = -1;
  }
...
int Screen::copyLineToStream(int line, int start, int count,
  ...
  if (line < history->getLines()) {
    ...
    if (history->isWrappedLine(line)) currentLineProperties |= LINE_WRAPPED;
  } else {
    if (count == -1) count = columns - start;
    ...
    count = qBound(0, count, qMax(length - start, 0));
...
void Screen::addHistLine() {
  ...
  if (hasScroll()) {
    ...
    // of dropped lines
    if (newHistLines == oldHistLines) _droppedLines++;
    ...
    if (selBegin != -1) {
      ...
      if (selTopLeft < top_BR) selTopLeft -= columns;

      if (selBottomRight < top_BR) selBottomRight -= columns;
...
void Screen::fillWithDefaultChar(Character* dest, int count) {
  for (int i = 0; i < count; i++) dest[i] = defaultChar;
}
