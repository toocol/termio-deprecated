...
void Vt102Emulation::addDigit(int digit) {
  if (argv[argc] < MAX_ARGUMENT) argv[argc] = 10 * argv[argc] + digit;
}
...
void Vt102Emulation::initTokenizer() {
  ...
  for (i = 0; i < 32; ++i) charClass[i] |= CTL;
  for (i = 32; i < 256; ++i) charClass[i] |= CHR;
  for (s = (quint8*)"@ABCDEFGHILMPSTXZbcdfry"; *s; ++s) charClass[*s] |= CPN;
  // resize = \e[8;<row>;<col>t
  for (s = (quint8*)"t"; *s; ++s) charClass[*s] |= CPS;
  for (s = (quint8*)"0123456789"; *s; ++s) charClass[*s] |= DIG;
  for (s = (quint8*)"()+*%"; *s; ++s) charClass[*s] |= SCS;
  for (s = (quint8*)"()+*#[]%"; *s; ++s) charClass[*s] |= GRP;
...
#define Xte    \
  (Xpe &&      \
   (cc == 7 || \
    (prevCC == 27 && cc == 92)))  // 27, 92 => "\e\\" (ST, String Terminator)
...
void Vt102Emulation::processWindowAttributeChange() {
  ...
  int i;
  for (i = 2;
       i < tokenBufferPos && tokenBuffer[i] >= '0' && tokenBuffer[i] <= '9';
       i++) {
  ...
  QString newValue =
      QString::fromWCharArray(tokenBuffer + i + 1, tokenBufferPos - i - 2);
...
void Vt102Emulation::sendMouseEvent(int cb, int cx, int cy, int eventType) {
  ...
  if (eventType == 2 && !getMode(MODE_Mouse1006)) cb = 3;
...
void Vt102Emulation::focusLost(void) {
  if (_reportFocusEvents) sendString("\033[O");
}
...
void Vt102Emulation::focusGained(void) {
  if (_reportFocusEvents) sendString("\033[I");
}
...
void Vt102Emulation::resetMode(int m) {
  ...
  switch (m) {
    case MODE_132Columns:
      if (getMode(MODE_Allow132Columns)) clearScreenAndSetColumns(80);
      break;
    case MODE_Mouse1000:
    case MODE_Mouse1001:
    case MODE_Mouse1002:
