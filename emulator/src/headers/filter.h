...
class Filter : public QObject {
 public:
   ...
   * method.  These actions may then be displayed in a popup menu or toolbar for
   * example.
   */
  class HotSpot {
   public:
    /**
     * Constructs a new hotspot which covers the area from (@p startLine,@p
     * startColumn) to (@p endLine,@p endColumn) in a block of text.
     */
    ...
    /**
     * Returns a list of actions associated with the hotspot which can be used
     * in a menu or toolbar
     */
  ...
  /** Causes the filter to process the block of text currently in its internal
   * buffer */
  ...
  /** Returns the list of hotspots identified by the filter which occur on a
   * given line */
...
class RegExpFilter : public Filter {
 public:
  ...
  class HotSpot : public Filter::HotSpot {
   public:
    ...
    /** Returns the texts found by the filter when matching the filter's regular
     * expression */
  ...
  /**
   * Sets the regular expression which the filter searches for in blocks of
   * text.
   *
   * Regular expressions which match the empty string are treated as not
   * matching anything.
   */
  void setRegExp(const QRegExp& text);
  /** Returns the regular expression which the filter searches for in blocks of
   * text */
 ...
 protected:
  /**
   * Called when a match for the regular expression is encountered.  Subclasses
   * should reimplement this to return custom hotspot types
   */
...
class UrlFilter : public RegExpFilter {
 ...
 public:
  /**
   * Hotspot type created by UrlFilter instances.  The activate() method opens a
   * web browser at the given URL when called.
   */
  class HotSpot : public RegExpFilter::HotSpot {
   public:
    ...
    /**
     * Open a web browser at the current URL.  The url itself can be determined
     * using the capturedTexts() method.
     */
    void activate(const QString& action = QString()) override;
...
class FilterChain : protected QList<Filter*> {
 public:
  ...
  /** Adds a new filter to the chain.  The chain will delete this filter when it
   * is destroyed */
  void addFilter(Filter* filter);
  /** Removes a filter from the chain.  The chain will no longer delete the
   * filter when destroyed */
  ...
  /** Returns the first hotspot which occurs at @p line, @p column or 0 if no
   * hotspot was found */
  Filter::HotSpot* hotSpotAt(int line, int column) const;
  /** Returns a list of all the hotspots in all the chain's filters */
  QList<Filter::HotSpot*> hotSpots() const;
  /** Returns a list of all hotspots at the given line in all the chain's
   * filters */
...
typedef TConsole::Filter Filter;
