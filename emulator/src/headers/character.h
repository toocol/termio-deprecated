...
class Character {
 public:
   ...
   * @param _r A set of rendition flags which specify how this character is to
   * be drawn.
   */
  inline Character(quint16 _c = ' ',
                   CharacterColor _f = CharacterColor(COLOR_SPACE_DEFAULT,
                                                      DEFAULT_FORE_COLOR),
                   CharacterColor _b = CharacterColor(COLOR_SPACE_DEFAULT,
                                                      DEFAULT_BACK_COLOR),
  ...
  /** A combination of RENDITION flags which specify options for drawing the
   * character. */
  quint8 rendition;
  ...
  /**
   * Compares two characters and returns true if they have the same unicode
   * character value, rendition and colors.
   */
  friend bool operator==(const Character& a, const Character& b);
  /**
   * Compares two characters and returns true if they have different unicode
   * character values, renditions or colors.
   */
  friend bool operator!=(const Character& a, const Character& b);
};
...
inline bool Character::isTransparent(const ColorEntry* base) const {
  return ((backgroundColor._colorSpace == COLOR_SPACE_DEFAULT) &&
          base[backgroundColor._u + 0 + (backgroundColor._v ? BASE_COLORS : 0)]
              .transparent) ||
...
inline ColorEntry::FontWeight Character::fontWeight(
    const ColorEntry* base) const {
  if (backgroundColor._colorSpace == COLOR_SPACE_DEFAULT)
    return base[backgroundColor._u + 0 + (backgroundColor._v ? BASE_COLORS : 0)]
        .fontWeight;
  else if (backgroundColor._colorSpace == COLOR_SPACE_SYSTEM)
    return base[backgroundColor._u + 2 + (backgroundColor._v ? BASE_COLORS : 0)]
        .fontWeight;
  else
...
class ExtendedCharTable {
 ...
 private:
  ...
  // in each value is the length of the buffer, followed by the ushorts in the
  // buffer themselves.
  QHash<ushort, ushort*> extendedCharTable;
};

}  // namespace TConsole
Q_DECLARE_TYPEINFO(TConsole::Character, Q_MOVABLE_TYPE);
