...
/**
 * Provides a window onto a section of a terminal screen.  A terminal widget can
 * then render the contents of the window and use the window to change the
 * terminal screen's selection in response to mouse or keyboard input.
 *
 * A new ScreenWindow for a terminal session can be created by calling
 * Emulation::createWindow()
 *
 * Use the scrollTo() method to scroll the window up and down on the screen.
 * Use the getImage() method to retrieve the character image which is currently
 * visible in the window.
 *
 * setTrackOutput() controls whether the window moves to the bottom of the
 * associated screen when new lines are added to it.
 *
 * Whenever the output from the underlying screen is changed, the
 * notifyOutputChanged() slot should be called.  This in turn will update the
 * window's position and emit the outputChanged() signal if necessary.
 */
class ScreenWindow : public QObject {
 ...
 public:
   ...
   * A screen must be specified by calling setScreen() before calling getImage()
   * or getLineProperties().
   *
   * You should not call this constructor directly, instead use the
   * Emulation::createWindow() method to create a window on the emulation which
   * you wish to view.  This allows the emulation to notify the window when the
   * associated screen has changed and synchronize selection updates between all
   * views on a session.
   */
   ...
   *
   * The returned buffer is managed by the ScreenWindow instance and does not
  ...
  /**
   * Returns true if the character at @p line , @p column is part of the
   * selection.
   */
   ...
   *
   * @param mode Specifies whether @p amount refers to the number of lines or
   * the number of pages to scroll.
   * @param amount The number of lines or pages ( depending on @p mode ) to
   * scroll by.  If this number is positive, the view is scrolled down.  If this
   * number is negative, the view is scrolled up.
   */
   ...
   * associated screen ( see screen() ) when the notifyOutputChanged() method is
   * called.
   */
 ...
 public slots:
   ...
   * have changed. This moves the window to the bottom of the screen if
   * trackOutput() is true and causes the outputChanged() signal to be emitted.
   */
 ...
 signals:
  /**
   * Emitted when the contents of the associated terminal screen (see screen())
   * changes.
   */
  void outputChanged();
 ...
 private:
  ...
  int _scrollCount;   // count of lines which the window has been scrolled by
                      // since the last call to resetScrollCount()
};
