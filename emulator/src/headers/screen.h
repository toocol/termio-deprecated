...
/**
   ...
   output. The scroll is specified using setScroll() The output history can be
   retrieved using writeToStream()
   ...
   visible image, characters which are part of the selection have their colours
   inverted.
*/
class Screen {
 public:
   ...
   * current cursor position with spaces. If @p n is 0 then one character is
   * erased.
   */
  ...
  /** Clear the whole screen, moving the current screen contents into the
   * history first. */
  ...
  // Do composition with last shown character FIXME: Not implemented yet for KDE
  // 4
   ...
   * being lost if the terminal display is resized smaller and then larger
   * again.
   *
   ...
   * The result is an array of Characters of size [getLines()][getColumns()]
   ...
   * characters into plain text with no formatting.
   * @param preserveLineBreaks Specifies whether new line characters should
   * be inserted into the returned text at the end of each terminal line.
   */
   ...
   *                   Double-height lines are formed of two lines containing
   * the same characters, with both having the LINE_DOUBLEHEIGHT attribute. This
   * allows other parts of the code to work on the assumption that all lines are
   * the same height.
   *
   * @param enable true to apply the attribute to the current line or false to
   * remove it
   */
 ...
 private:
  ...
  // the end of the line
  int copyLineToStream(int line, int start, int count,
                       TerminalCharacterDecoder* decoder, bool appendNewLine,
                       bool preserveLineBreaks) const;

  // fills a section of the screen image with the character 'c'
  // the parameters are specified as offsets from the start of the screen image.
  // the loc(x,y) macro can be used to generate these values from a column,line
  // pair.
