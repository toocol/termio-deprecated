...
bool KPtyDevicePrivate::_k_canWrite() {
  ...
  if (writeBuffer.isEmpty()) return false;

  qt_ignore_sigpipe();
  int wroteBytes;
  NO_INTR(wroteBytes, write(q->masterFd(), writeBuffer.readPointer(),
                            writeBuffer.readSize()));
  if (wroteBytes < 0) {
  ...
  if (!writeBuffer.isEmpty()) writeNotifier->setEnabled(true);
  return true;
#endif
  return true;
}
...
bool KPtyDevicePrivate::doWait(int msecs, bool reading) {
  ...
  while (reading ? readNotifier->isEnabled() : !writeBuffer.isEmpty()) {
    ...
    if (readNotifier->isEnabled()) FD_SET(q->masterFd(), &rfds);
    if (!writeBuffer.isEmpty()) FD_SET(q->masterFd(), &wfds);
    ...
    switch (select(q->masterFd() + 1, &rfds, &wfds, nullptr, tvp)) {
      ...
      default:
        if (FD_ISSET(q->masterFd(), &rfds)) {
          ...
          if (reading && canRead) return true;
        }
        if (FD_ISSET(q->masterFd(), &wfds)) {
          bool canWrite = _k_canWrite();
          if (!reading) return canWrite;
        }
        break;
    }
  }
