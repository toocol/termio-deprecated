...
void KeyboardTranslatorManager::findTranslators() {
  ...
  while (listIter.hasNext()) {
    ...
    if (!_translators.contains(name)) _translators.insert(name, 0);
  }

  _haveLoadedAll = true;
}

const KeyboardTranslator* KeyboardTranslatorManager::findTranslator(
    const QString& name) {
  if (name.isEmpty()) return defaultTranslator();
...
bool KeyboardTranslatorManager::saveTranslator(
...
KeyboardTranslator* KeyboardTranslatorManager::loadTranslator(
    const QString& name) {
...
const KeyboardTranslator* KeyboardTranslatorManager::defaultTranslator() {
  ...
  const KeyboardTranslator* translator =
      findTranslator(QLatin1String("default"));
  if (!translator) {
    QBuffer textBuffer;
...
KeyboardTranslator* KeyboardTranslatorManager::loadTranslator(
    QIODevice* source, const QString& name) {
  KeyboardTranslator* translator = new KeyboardTranslator(name);
  KeyboardTranslatorReader reader(source);
  translator->setDescription(reader.description());
  while (reader.hasNextEntry()) translator->addEntry(reader.nextEntry());
...
KeyboardTranslatorWriter::~KeyboardTranslatorWriter() { delete _writer; }
void KeyboardTranslatorWriter::writeHeader(const QString& description) {
  *_writer << "keyboard \"" << description << '\"' << '\n';
}
void KeyboardTranslatorWriter::writeEntry(
    const KeyboardTranslator::Entry& entry) {
  QString result;
...
void KeyboardTranslatorReader::readNext() {
  ...
  while (!_source->atEnd()) {
    const QList<Token>& tokens =
        tokenize(QString::fromUtf8(_source->readLine()));
    if (!tokens.isEmpty() && tokens.first().type == Token::KeyKeyword) {
      ...
      decodeSequence(tokens[1].text.toLower(), keyCode, modifiers, modifierMask,
                     flags, flagMask);
...
bool KeyboardTranslatorReader::parseAsCommand(
  ...
  else if (text.compare(QLatin1String("scrollpageup"), Qt::CaseInsensitive) ==
           0)
    command = KeyboardTranslator::ScrollPageUpCommand;
  else if (text.compare(QLatin1String("scrollpagedown"), Qt::CaseInsensitive) ==
           0)
    command = KeyboardTranslator::ScrollPageDownCommand;
  else if (text.compare(QLatin1String("scrolllineup"), Qt::CaseInsensitive) ==
           0)
    command = KeyboardTranslator::ScrollLineUpCommand;
  else if (text.compare(QLatin1String("scrolllinedown"), Qt::CaseInsensitive) ==
           0)
  ...
  else if (text.compare(QLatin1String("scrolluptotop"), Qt::CaseInsensitive) ==
           0)
    command = KeyboardTranslator::ScrollUpToTopCommand;
  else if (text.compare(QLatin1String("scrolldowntobottom"),
  ...
  for (int i = 0; i < text.count(); i++) {
    ...
    if ((endOfItem || isLastLetter) && !buffer.isEmpty()) {
      ...
      if (parseAsModifier(buffer, itemModifier)) {
        ...
        if (isWanted) tempModifiers |= itemModifier;
      } else if (parseAsStateFlag(buffer, itemFlag)) {
        tempFlagMask |= itemFlag;

        if (isWanted) tempFlags |= itemFlag;
...
bool KeyboardTranslatorReader::parseAsStateFlag(
    const QString& item, KeyboardTranslator::State& flag) {
  if (item == QLatin1String("appcukeys") ||
  ...
  else if (item == QLatin1String("anymod") ||
...
KeyboardTranslator::Entry KeyboardTranslatorReader::createEntry(
    const QString& condition, const QString& result) {
  ...
  if (reader.hasNextEntry()) entry = reader.nextEntry();
...
bool KeyboardTranslatorReader::parseError() { return false; }
QList<KeyboardTranslatorReader::Token> KeyboardTranslatorReader::tokenize(
    const QString& line) {
  QString text = line;
  ...
  if (commentPos != -1) text.remove(commentPos, text.length());
...
bool KeyboardTranslator::Entry::matches(int keyCode,
  ...
  if (_keyCode != keyCode) return false;

  if ((modifiers & _modifierMask) != (_modifiers & _modifierMask)) return false;

  // if modifiers is non-zero, the 'any modifier' state is implicit
  if ((modifiers & ~Qt::KeypadModifier) != 0) testState |= AnyModifierState;

  if ((testState & _stateMask) != (_state & _stateMask)) return false;
  ...
  if (_stateMask & KeyboardTranslator::AnyModifierState) {
    if (wantAnyModifier != anyModifiersSet) return false;
  }

  return true;
}
QByteArray KeyboardTranslator::Entry::escapedText(
  ...
  for (int i = 0; i < result.count(); i++) {
    ...
    switch (ch) {
      ...
      default:
        ...
        if (!QChar(QLatin1Char(ch)).isPrint()) replacement = 'x';
    }
...
QByteArray KeyboardTranslator::Entry::unescape(const QByteArray& input) const {
  ...
  for (int i = 0; i < result.count() - 1; i++) {
    ...
    if (ch == '\\') {
      ...
      if (escapedChar) result.replace(i, charsToRemove, replacement);
    }
  }
...
void KeyboardTranslator::Entry::insertModifier(QString& item,
                                               int modifier) const {
  if (!(modifier & _modifierMask)) return;
...
void KeyboardTranslator::replaceEntry(const Entry& existing,
  ...
  if (!existing.isNull()) _entries.remove(existing.keyCode(), existing);
...
KeyboardTranslator::Entry KeyboardTranslator::findEntry(
    int keyCode, Qt::KeyboardModifiers modifiers, States state) const {
  for (auto it = _entries.cbegin(), end = _entries.cend(); it != end; ++it) {
    if (it.key() == keyCode)
      if (it.value().matches(keyCode, modifiers, state)) return *it;
  }
