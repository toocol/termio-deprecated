...
// HistoryFile::map() should only map in sections of the file at a time, to
// avoid
// this.
void HistoryFile::map() {
  ...
  if (fileMap == MAP_FAILED) {
    readWriteBalance = 0;
    fileMap = nullptr;
    // qDebug() << __FILE__ << __LINE__ << ": mmap'ing history failed.  errno =
    // " << errno;
  }
}
...
bool HistoryFile::isMapped() const { return (fileMap != nullptr); }
...
void HistoryFile::get(unsigned char* bytes, int len, int loc) {
  ...
  readWriteBalance--;
  if (!fileMap && readWriteBalance < MAP_THRESHOLD) map();

  if (fileMap) {
    for (int i = 0; i < len; i++) bytes[i] = fileMap[loc + i];
  } else {
    int rc = 0;
...
int HistoryFile::len() { return length; }

// History Scroll abstract base class //////////////////////////////////////

HistoryScroll::HistoryScroll(HistoryType* t) : m_histType(t) {}

HistoryScroll::~HistoryScroll() { delete m_histType; }

bool HistoryScroll::hasScroll() { return true; }
...
int HistoryScrollFile::getLines() { return index.len() / sizeof(int); }
...
HistoryScrollBuffer::~HistoryScrollBuffer() { delete[] _historyBuffer; }

void HistoryScrollBuffer::addCellsVector(const QVector<Character>& cells) {
  _head++;
  if (_usedLines < _maxLineCount) _usedLines++;
...
int HistoryScrollBuffer::getLines() { return _usedLines; }
...
int HistoryScrollBlockArray::getLines() { return m_lineLengths.count(); }
...
bool HistoryScrollBlockArray::isWrappedLine(int /*lineno*/) { return false; }
...
void* CompactHistoryBlock::allocate(size_t length) {
  Q_ASSERT(length > 0);
  if (tail - blockStart + length > blockLength) return nullptr;
...
void* CompactHistoryBlockList::allocate(size_t size) {
  ...
  if (list.isEmpty() || list.last()->remaining() < size) {
    ...
    list.append(block);
    // kDebug() << "new block created, remaining " << block->remaining() <<
...
void* CompactHistoryLine::operator new(size_t size,
...
CompactHistoryLine::CompactHistoryLine(const TextLine& line,
  ...
  if (!line.empty()) {
    ...
    formatArray = (CharacterFormat*)blockList.allocate(sizeof(CharacterFormat) *
                                                       formatLength);
    ...
    while (k < length && j < formatLength) {
      if (!(line[k].equalsFormat(c))) {
        ...
        // kDebug() << "format entry " << j << " at pos " <<
        // formatArray[j].startPos << " " << &(formatArray[j].startPos) ;
        j++;
      }
      k++;
    }
...
int CompactHistoryScroll::getLines() { return lines.size(); }
...
HistoryScroll* HistoryTypeBuffer::scroll(HistoryScroll* old) const {
  if (old) {
    ...
    int startLine = 0;
    if (lines > (int)m_nbLines) startLine = lines - m_nbLines;
...
HistoryScroll* HistoryTypeFile::scroll(HistoryScroll* old) const {
  if (dynamic_cast<HistoryFile*>(old)) return old;  // Unchanged.
