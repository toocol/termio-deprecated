...
KProcess::~KProcess() { delete d_ptr; }
...
void KProcess::setEnv(const QString &name, const QString &value,
                      bool overwrite) {
...
void KProcess::unsetEnv(const QString &name) {
  ...
  for (QStringList::Iterator it = env.begin(); it != env.end(); ++it)
    if ((*it).startsWith(fname)) {
      env.erase(it);
      if (env.isEmpty()) env.append(QString::fromLatin1(DUMMYENV));
      setEnvironment(env);
      return;
    }
}
...
int KProcess::startDetached(const QString &exe, const QStringList &args) {
  qint64 pid;
  if (!QProcess::startDetached(exe, args, QString(), &pid)) return 0;
  return (int)pid;
}
