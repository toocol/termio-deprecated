...
ScreenWindow::ScreenWindow(QObject* parent)
    : QObject(parent),
      ...
      _trackOutput(true),
      _scrollCount(0) {}
ScreenWindow::~ScreenWindow() { delete[] _windowBuffer; }
...
Screen* ScreenWindow::screen() const { return _screen; }
...
Character* ScreenWindow::getImage() {
  ...
  if (!_bufferNeedsUpdate) return _windowBuffer;
...
QVector<LineProperty> ScreenWindow::getLineProperties() {
  ...
  if (result.count() != windowLines()) result.resize(windowLines());
...
void ScreenWindow::setSelectionStart(int column, int line, bool columnMode) {
  _screen->setSelectionStart(
      column, qMin(line + currentLine(), endWindowLine()), columnMode);
...
int ScreenWindow::windowLines() const { return _windowLines; }

int ScreenWindow::windowColumns() const { return _screen->getColumns(); }

int ScreenWindow::lineCount() const {
  return _screen->getHistLines() + _screen->getLines();
}

int ScreenWindow::columnCount() const { return _screen->getColumns(); }
...
void ScreenWindow::notifyOutputChanged() {
  ...
  if (_trackOutput) {
    ...
    _currentLine = qMax(
        0, _screen->getHistLines() - (windowLines() - _screen->getLines()));
  } else {
