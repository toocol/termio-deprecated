...
void FilterChain::addFilter(Filter* filter) { append(filter); }
void FilterChain::removeFilter(Filter* filter) { removeAll(filter); }
bool FilterChain::containsFilter(Filter* filter) { return contains(filter); }
...
void FilterChain::setBuffer(const QString* buffer,
  ...
  QListIterator<Filter*> iter(*this);
  while (iter.hasNext()) iter.next()->setBuffer(buffer, linePositions);
}
void FilterChain::process() {
  QListIterator<Filter*> iter(*this);
  while (iter.hasNext()) iter.next()->process();
}
void FilterChain::clear() { QList<Filter*>::clear(); }
...
void Filter::getLineColumn(int position, int& startLine, int& startColumn) {
  ...
  for (int i = 0; i < _linePositions->count(); i++) {
    ...
    if (_linePositions->value(i) <= position && position < nextLine) {
      startLine = i;
      startColumn = string_width(buffer()
...
const QString* Filter::buffer() { return _buffer; }
...
QList<Filter::HotSpot*> Filter::hotSpots() const { return _hotspotList; }
...
Filter::HotSpot* Filter::hotSpotAt(int line, int column) const {
  ...
  while (spotIter.hasNext()) {
    ...
    if (spot->startLine() == line && spot->startColumn() > column) continue;
    if (spot->endLine() == line && spot->endColumn() < column) continue;
...
Filter::HotSpot::HotSpot(int startLine, int startColumn, int endLine,
                         int endColumn)
...
void RegExpFilter::setRegExp(const QRegExp& regExp) { _searchText = regExp; }
QRegExp RegExpFilter::regExp() const { return _searchText; }
...
void RegExpFilter::process() {
  ...
  if (_searchText.exactMatch(emptyString)) return;
  ...
  while (pos >= 0) {
    ...
    if (pos >= 0) {
      ...
      if (_searchText.matchedLength() == 0) pos = -1;
    }
  }
}
...
const QRegExp UrlFilter::FullUrlRegExp(QLatin1String(
    "(www\\.(?!\\.)|[a-z][a-z0-9+.-]*://)[^\\s<>'\"]+[^!,\\.\\s<>'\"\\]]"));
// email address:
...
UrlFilter::HotSpot::~HotSpot() { delete _urlObject; }

void FilterObject::emitActivated(const QUrl& url, bool fromContextMenu) {
  emit activated(url, fromContextMenu);
}

void FilterObject::activate() { _filter->activate(sender()->objectName()); }
