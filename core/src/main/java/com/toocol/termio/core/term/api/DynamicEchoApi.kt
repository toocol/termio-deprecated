...
object DynamicEchoApi : SuspendApi {
    ...
    suspend fun dynamicEcho(cmd: String) {
        ...
        try {
            ...
            if (command == null) {
                ...
                if (StringUtils.isNumeric(finalCmd)) {
                    ...
                    if (credential == null) {
                        ...
                        val status =
                            if (sshSessionCache.isAlive(credential.host)) ColorHelper.front("alive",
                                Term.theme.sessionAliveColor.color) else "offline"
                        connectionPrompt
            ...
            error("Dynamic echo error, e = ${e.javaClass.name}")
        } finally {
            running = false
        }
    }

    private fun spaceProcess(cmd: String) {
        val commandHighlightColor = Term.theme.commandHighlightColor.color
        if (cmd.contains(StrUtil.SPACE)) {
            ...
            if (splitCommand == null) {
                ...
                val alikeCommand = TermCommand.findAlike(split[0])
