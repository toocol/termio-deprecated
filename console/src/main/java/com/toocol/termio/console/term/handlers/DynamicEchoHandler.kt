...
class DynamicEchoHandler(vertx: Vertx?, context: Context?) : NonBlockingMessageHandler(
    ...
    override fun <T> handleInline(message: Message<T>) {
        ...
        val backgroundColor = Term.theme.displayBackGroundColor.color
        ...
        if (command == null) {
            ...
            if (StringUtils.isNumeric(finalCmd)) {
                val connectionPrompt = AnsiStringBuilder().background(backgroundColor)
            ...
            if (StringUtils.isNotEmpty(command.specify)) {
                ...
                val builder = AnsiStringBuilder().background(backgroundColor)
    ...
    private fun spaceProcess(term: Term, cmd: String) {
        ...
        if (cmd.contains(StrUtil.SPACE)) {
            ...
            if (splitCommand == null) {
                val printMsg = AnsiStringBuilder().background(backgroundColor)
                    .append("Command not found: ")
            ...
            val titleMsg = AnsiStringBuilder().background(backgroundColor).append("Alternative commands: ")
            val printMsg = AnsiStringBuilder().background(backgroundColor)
